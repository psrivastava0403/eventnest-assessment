...
RSpec.describe "Orders API", type: :request do
  ...
  describe "GET /api/v1/orders" do
    it "returns orders" do
      create(:order, user: attendee, event: event, total_amount: 100.0)
  ...
  describe "GET /api/v1/orders/:id" do
    it "returns order details" do
      order = create(:order, user: attendee, event: event, total_amount: 100.0)
  ...
  describe "POST /api/v1/orders/:id/cancel" do
    it "cancels a pending order" do
      order = create(:order, user: attendee, event: event, status: "pending", total_amount: 100.0)
